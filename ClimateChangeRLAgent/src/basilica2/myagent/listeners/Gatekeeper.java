...
public class Gatekeeper extends BasilicaAdapter
{
	...
	public void readyUser(InputCoordinator source, String username)
	{
		...
		else
		{
			...
			source.addEventProposal("macro", new PrivateMessageEvent(source, username, getAgent().getName(), prompter.lookup("ACKNOWLEDGE", slots), "ACKNOWLEDGE"));
		}
	}
	...
	public void allReady(InputCoordinator source)
	{
		...
		remainingKeys.clear();
		source.pushEventProposal("macro", new MessageEvent(source, getAgent().getUsername(), prompter.lookup("ALL_READY", slots), "ALL_READY"), 1.0, 2);
		source.pushEventProposal("macro", new StepDoneEvent(source, stepName), 1.0, 10);
		source.addEventProposal("macro", new ReadyEvent(source, false), 1.0, 60);
	}
