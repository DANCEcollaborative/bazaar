...
public class AskForExplanationAnnotator extends BasilicaAdapter
{
    ...
    int chosenEngagementLevel = 3;
    String[] explainMoreAnnotatorTrueThreshold;
    
	public AskForExplanationAnnotator(Agent a)
	{
		super(a);
		...
		explainMoreAnnotatorTrueThreshold = RLproperties.getProperty("explainmore_annotator_true_threshold", "70,60,50,40,30").split(",");
		String[] classificationList = classificationString.split(","); 
		if(chosenEngagementLevel<explainMoreAnnotatorTrueThreshold.length) {
			classificationList[1]=explainMoreAnnotatorTrueThreshold[chosenEngagementLevel];
		}
		...
		try {
			...
			if (isAlive) {
				System.err.println("AskForExplanationAnnotator LightSide process is alive");
			}
			else {
				System.err.println("AskForExplanationAnnotator LightSide process is NOT alive");			
			}
			
		} 
		catch (Exception e)
		{
			System.err.println("AskForExplanationAnnotator, error starting LightSide");
			e.printStackTrace();
		}
	...
	public String annotateText(String text)
	{
		...
		try {
	    	...
	    	e.printStackTrace();
	    	return "AskForExplanationAnnotator LightSide returned null"; 
	    }	
	}
	...
	public String parseLightSideResponse(StringBuilder response)
	{
		...
		for (int i=0; i < listLength; i++) {
			...
			System.err.println("=== AskForExplanationAnnotator - classification " + classification + " " + Double.toString(classificationPercent) + "%");
			try {
				classificationThreshold = classify_dict.get(classification);
				System.err.println("AskForExplanationAnnotator, classification="+classification+" classificationThreshold="+classificationThreshold);
		    	...
		    	System.err.println("AskForExplanationAnnotator LightSide classification \"" + classification + "\" not used"); 
			}			
		}
	...
	public static void main(String[] args)
	{
		...
		AskForExplanationAnnotator annotator = new AskForExplanationAnnotator(null);
