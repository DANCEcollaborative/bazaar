...
function setTeam(teamNumber,req,provider,logger,res)
{
	...
	var url = 'http://bazaar.lti.cs.cmu.edu:8000/chat/'+ roomname  + '/' + req.query.mturkid + '/' + 
                                                             req.query.username + '/' + req.query.perspective + '/';
...
io.sockets.on('connection', function (socket) {
	...
	socket.on('adduser', function(room, username, temporary, id, perspective){
	   ...
	   if(isBlank(room))
	    ...
	    loadHistory(socket, false);
            console.log(username,id, perspective);
	...
	socket.on('disconnect', function()
	{
            ...
            if(socket.username != "BazaarAgent" && socket.room in numUsers)
            {
