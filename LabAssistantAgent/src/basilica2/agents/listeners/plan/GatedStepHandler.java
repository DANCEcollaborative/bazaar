...
//		Logger.commonLog(getClass().getSimpleName(),Logger.LOG_NORMAL,"starting gated step...");
        ...
        if(currentStep.attributes.containsKey("required_participants"))
        {
            ...
            {
                Logger.commonLog(getClass().getSimpleName(), Logger.LOG_WARNING, "Invalid required_participants value for " + currentStep.name);
            }
        }
        ...
        {
            String promptId = currentStep.name;
            if(currentStep.attributes.containsKey("prompt"))
            {
                promptId = currentStep.attributes.get("prompt");
            }
            String text = prompter.lookup(promptId);
            MessageEvent message = new MessageEvent(source, overmind.getAgent().getUsername(), text, promptId);
            source.pushProposal(PriorityEvent.makeBlackoutEvent("macro", "PromptStep", message, OutputCoordinator.HIGH_PRIORITY, 5.0, 3));
        }
        else
        {
