			...
			if (isRunning)
			{
				...
				if (steps.size() > 0)
				{
					currentStep = steps.remove(0);
 					System.err.println("Stage.java progressStage: currentStep removed and to be executed: " + currentStep.toString()); 
 					Logger.commonLog(this.getClass().getName(), Logger.LOG_NORMAL, "Stage.java progressStage: currentStep removed and to be executed: \" + currentStep.toString()"); 
				...
				{
					Logger.commonLog(this.getClass().getName(), Logger.LOG_NORMAL,"Stage.java progressStage: finished stage "+this.name);
 					System.err.println("Stage.java progressStage: finished stage "+this.name);
					if (nextStage != null)
					{
						if (delay == 0) {
							planExecutor.activateStage(nextStage);
						} else {
							Logger.commonLog(this.getClass().getName(), Logger.LOG_NORMAL, "Stage.java progressStage: " + delay + " second delay before next stage = "+nextStage);
 							System.err.println("Stage.java progressStage: " + delay + " second delay before next stage = "+nextStage);
