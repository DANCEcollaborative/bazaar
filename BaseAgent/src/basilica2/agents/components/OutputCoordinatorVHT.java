 ...
 * @author dadamson
 */
public class OutputCoordinatorVHT extends OutputCoordinator  // implements TimeoutReceiver
{
	...
	private VHSender sender = new VHSender();

	public OutputCoordinatorVHT(Agent agent, String s1, String s2)
	{
	...
	private void publishMessage(MessageEvent me)
	{
		...
		// log(Logger.LOG_NORMAL, "*** OutputCoordinatorVHT, entire message ***: " + me.getText());
		
		if (!me.getText().contains("|"))
		{
			// log(Logger.LOG_NORMAL, "-- OutputCoordinatorVHT, single message ---: " + me.getText());
			broadcast(me);
			sender.sendMessage(me.getText());
		...
		else
		{
			...
			for (int i = 0; i < messageParts.length; i++)
			{
				...
				// log(Logger.LOG_NORMAL, "-- OutputCoordinatorVHT, partial message ---: " + newme.getText());
				broadcast(newme);
				sender.sendMessage(newme.getText());
