...
class RangeSliderUI extends BasicSliderUI
{
	...
	private Color upperColor = new Color(128, 192, 255);
	private Color lowerColor = new Color(255, 192, 128);
	...
	private transient boolean lowerPressed = false;
	private transient boolean upperPressed = false;
	...
	public void paintTrack(Graphics g)
	{
		...
		if (slider.getOrientation() == JSlider.HORIZONTAL)
		{
			...
			int rangeX = lowerX - trackBounds.x;
			int rangeY = -thumbRect.height/2; //trackBounds.y - trackBounds.height / 2;
			int rangeWidth = upperX - lowerX;
			g.fillRect(rangeX, rangeY, rangeWidth, trackBounds.height);

			g.fillOval(rangeX - thumbRect.width / 2, rangeY, thumbRect.width - 1, trackBounds.height);
			g.fillOval(rangeWidth + rangeX - thumbRect.width / 2, rangeY, thumbRect.width - 1, trackBounds.height);
	...
	private void paintLowerThumb(Graphics g)
	{
		...
		else
		{
			...
			g2d.setColor(lowerColor.darker());
			g2d.draw(thumbShape);
		}
	...
	private void paintUpperThumb(Graphics g)
	{
		...
		else
		{
			...
			g2d.setColor(upperColor.darker());
			g2d.draw(thumbShape);
		}
	...
	public class RangeTrackListener extends TrackListener
	{
		...
		{
			int offsetLow=0, offsetHigh=0;
			
			switch (slider.getOrientation())
			{
				case JSlider.VERTICAL:
					offsetHigh = currentMouseY - upperThumbRect.y;
					offsetLow = currentMouseY - thumbRect.y;
					break;
				case JSlider.HORIZONTAL:
					offsetHigh = currentMouseX - upperThumbRect.x;
					offsetLow = currentMouseX - thumbRect.x;
					break;
			}
			...
			else if(Math.abs(offsetHigh) < Math.abs(offsetLow))
			{
				moveUpperThumb();
			}
			else
