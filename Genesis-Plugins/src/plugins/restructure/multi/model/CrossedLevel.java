...
public class CrossedLevel extends StructuredLevel {

	public CrossedLevel(StructuredLevel a, StructuredLevel b){
		classA = a;
		classB = b;
	}
	...
	public Collection<FeatureHit> model(FeatureTable table, RestructurePlugin plugin) {
		...
		hits.addAll(super.model(table, plugin));
		return hits;
	}
	...
	public String getPrefix(FeatureTable table, int index) {
		return classA.getPrefix(table, index)+"_"+classB.getPrefix(table, index);
	}
