...
import basilica2.myagent.listeners.Gatekeeper;
import basilica2.agents.listeners.plan.*;
...
public class GatedStepHandler implements StepHandler
{
	...
	public void execute(final Step currentStep, final PlanExecutor overmind, final InputCoordinator source)
	{
		...
		new Timer(Math.max(currentStep.timeout - 180, currentStep.timeout*0.75), currentStep.name, new TimeoutAdapter()
		{
			...
			{
//				overmind.currentPlan.currentStage.currentStep;
				String[] splits = overmind.getStatus().split(" > ");
				String latest_step_name = splits[splits.length-1];
				if((currentStep.name).equals(latest_step_name)) //the plan has not progressed on its own yet
				{
