...
public class ChatterBoxActor extends BasilicaAdapter
{
	...
	public ChatterBoxActor(Agent a)
	{
		super(a);
		...
		if(fp.exists())
		{
			...
			try
			{
				String timeString = rows.get(0).get("TIME");
				zero = transcriptFormat.parse(timeString);
				start = Long.parseLong(System.getProperty("chatterbox.start_time", ""+Timer.currentTimeMillis()));
				
				
				offset = start - zero.getTime();
				log(Logger.LOG_NORMAL, agentName+"\tZERO:\t"+zero.getTime()+ "="+timeString);
				log(Logger.LOG_NORMAL, agentName+"\tSTART:\t"+start);
				log(Logger.LOG_NORMAL, agentName+"\tOFFSET:\t"+offset);
			}
			catch (Exception e)
			{e.printStackTrace();}
			...
			while(rowIt.hasNext())
			{
				...
				if(!row.get("AUTHOR").equals(agentName) 
						|| row.get("TEXT").equals("joins the room")
						|| row.get("TEXT").equals("leaves the room"))
					rowIt.remove();
			}
		}
		else System.err.println("no transcript file "+fp);
	}
	...
	public void processEvent(final InputCoordinator source, Event event)
	{
		if(!started && !rows.isEmpty())
		{
			...
			new Timer(time, new TimeoutReceiver()
			{
				...
				{
					String text = rows.get(i).get("TEXT");
					ChatterBoxActor.this.log(Logger.LOG_NORMAL, (agentName+":\tROW "+i+" TEXT="+text));
					source.pushEventProposal(new MessageEvent(source, getAgent().getName(), text));
					i++;
		...
		else ChatterBoxActor.this.log(Logger.LOG_ERROR, "can't start chatterbox actor "+agentName+": "+rows);
	}
...
//		{
//			double time=(offset + transcriptFormat.parse(rows.get(i).get("TIME")).getTime() - Timer.currentTimeMillis())/1000.0;
//			log(Logger.LOG_NORMAL, agentName+":\tSCHEDULING ROW "+i+" FOR "+time);
//			return time;
//		}
//		catch (ParseException e)
//		{
