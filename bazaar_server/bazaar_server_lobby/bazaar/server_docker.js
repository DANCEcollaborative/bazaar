...
app.use(function(req, res, next) {
    res.setHeader("Content-Security-Policy", "default-src 'self' https://erebor.lti.cs.cmu.edu:9001/p/ https://bazaar.lti.cs.cmu.edu/bazaar/vertical.pdf https://bazaar.lti.cs.cmu.edu/bazaar/horizontal.pdf; connect-src 'self' ws://bazaar.lti.cs.cmu.edu/bazsocket/ http://bazaar.lti.cs.cmu.edu/bazsocket/ wss://bazaar.lti.cs.cmu.edu/bazsocket/ https://bazaar.lti.cs.cmu.edu/bazsocket/; style-src 'self' https://fonts.googleapis.com/css https://cdn.jsdelivr.net/gh/DANCECollaborative/bazaar@latest/bazaar_server/bazaar_server_lobby/bazaar/discussionnew2.css https://rawgit.com/gtomar/help-button-javascript/master/discussion.css 'unsafe-inline'; script-src 'self' https://ajax.googleapis.com/ajax/libs/jquery/1.7.1/jquery.min.js https://rawgit.com/gtomar/stylesheets/master/colors.js https://rawgit.com/gtomar/stylesheets/master/jquery.sortable.js https://rawgit.com/marinawang/bazaar/master/client.js https://cdnjs.cloudflare.com/ajax/libs/socket.io/2.3.0/socket.io.js 'unsafe-inline'; font-src 'self' https://fonts.gstatic.com/s/oxygen/v10/2sDfZG1Wl4LcnbuKjk0mRUe0Aw.woff2 https://fonts.gstatic.com/s/oxygen/v10/2sDfZG1Wl4LcnbuKgE0mRUe0A4Uc.woff2; img-src http://www.dnr.sc.gov/climate/sco/Education/wxmap/wxmap.gif http://bazaar.lti.cs.cmu.edu/favicon.ico"); 
    return next();
});
			...
			connection.query('update nodechat.room_prefix set count='+teamNumber+' where name='+connection.escape(req.query.roomName), function(err, rows, fields)
		            {
				        ...
				        numUsers[req.query.roomName + pad(teamNumber, 2)] = 0;
                        		exec("../lobby/launch_agent_docker.sh " + req.query.roomName + " " + teamNumber + ' "none"', puts);
                        		sleep(5000);
					setTeam(teamNumber,req,provider,logger,res);
                                }
			    });
...
function setTeam_(teamNumber,req,provider,logger,res)
{
        if( (!(req.query.roomName + teamNumber in numUsers)) )
        {
                ...
                console.log("../lobby/launch_agent_docker.sh " + req.query.roomName + " " + teamNumber + ' "none"', puts);
                exec("../lobby/launch_agent_docker.sh " + req.query.roomName + " " + teamNumber + ' "none"', puts);
                sleep(5000);
        }
        //teamNumber = req.query.roomId;
