...
public class SynonymSentenceMatcher extends ASentenceMatcher
{
	...
	private static SynonymSentenceMatcher sentenceMatcher;
	...
	public SynonymSentenceMatcher(String dictPath, String stopwordPath)
	{
	...
	public SynonymSentenceMatcher(String stopwordPath)
	{
		super(stopwordPath);
	}
	...
	public double getSynonymSimilarity(String one, String two)
	{
		...
		sim = Math.pow(2 * sim / (lemmas.size() + lemmas2.size()), 0.5);
	...
	public double getSentenceSimilarity(String incoming, String target, double lengthThreshold, boolean polarityMustMatch, boolean removeStopwords)
	{
		...
		{
			if (verbose > 2) System.out.println("sentence word counts differ too much: " + lengthRatio);
			return 0.0;
		}
		...
		for (String t : allTokens)
		{
			...
			if (contentWords.contains(t) || isSpecial(t))// &&
															// target.contains(t))
			{

				if (wsim1 < 0.3 || wsim2 < 0.3)
				{
					if (verbose > 2) System.out.println("\t\tno match for content word " + t + ": " + Math.min(wsim1, wsim2));
					//wsim1 -= CONTENT_PENALTY;
					sim -= CONTENT_PENALTY; //super harsh
				}
	...
	public static void main(String[] args) throws Exception
	{
		...
		SynonymSentenceMatcher matcher = new SynonymSentenceMatcher("chemistry/chem_synonyms.txt", "chemistry/stopwords.txt");
	...
	public static SynonymSentenceMatcher getInstance()
	{
		if (sentenceMatcher == null)
		{
			...
			sentenceMatcher = new SynonymSentenceMatcher("data/synonymizer/stopwords.txt");
		}
